...
backend/app/
  ...
  runtime/
    ...
    langgraph/runtime_policy.py    # 执行策略解释（standard/quick + depth）
    langgraph/budgeting.py         # token/超时/轮次预算
    strategy_center.py             # runtime_strategy 选择
    deployment_center.py           # deployment_profile 选择
...
- `backend/app/services/debate_service.py`：业务主入口，组织上下文、资产映射、执行与结果落库。
- `backend/app/runtime/langgraph_runtime.py`：运行时 orchestrator，负责图构建、状态初始化、执行与事件派发。
- `backend/app/runtime/langgraph/builder.py`：LangGraph 图定义入口（节点与边）。
- `backend/app/runtime/langgraph/nodes/supervisor.py`：主 Agent 的命令与路由入口。
- `backend/app/runtime/langgraph/nodes/agents.py`：专家 Agent 执行壳（上下文组装、调用 LLM、结构化输出）。
- `backend/app/runtime/langgraph/phase_executor.py`：每阶段执行器（并行分析、critique、judgment 等）。
- `backend/app/runtime/langgraph/runtime_policy.py`：解释执行模式与深度，输出策略。
- `backend/app/runtime/langgraph/budgeting.py`：token/超时/轮次预算策略。
- `backend/app/runtime/strategy_center.py`：选择 `runtime_strategy`（balanced/low_cost）。
...
从最新实现看，运行时把“分析策略”和“投递方式”明确拆开：

- `execution_mode`：分析策略，仅用于路由与预算（`standard / quick`）。
- `requested_execution_mode`：用户原始选择，用于历史页与审计展示，不应被后台投递覆盖。
- `execution_delivery_mode`：投递方式（`foreground / background`），只影响任务执行方式。

也就是说：**`background` 不再是分析策略**，后台执行只改 `execution_delivery_mode`，不会覆盖 `execution_mode`。

```mermaid
flowchart TD
    U["用户选择模式"] --> R["requested_execution_mode"]
    R --> E["execution_mode (分析策略)"]
    R --> D["execution_delivery_mode (投递方式)"]
    D -->|foreground| F1["同步执行 / 前台展示"]
    D -->|background| F2["后台执行 / 可恢复"]
```
...
- `JudgeCoverageRule`
- `StandardSecondRoundTargetedRule`（标准模式第 2 轮后优先定向补证）

`StandardSecondRoundTargetedRule` 的目的：
- 避免标准模式反复触发“全量并行分析”。
- 优先依据 `round_gap_summary` 与 Judge 的 `next_checks/action_items` 进行定向补证。
- 若无明确缺口，直接进入 Judge 收口。
...
### 4.3.1 标准模式第 2 轮后的定向补证

当前标准模式在第 2 轮以后不再重复“全量并行分析”，而是优先做定向补证：

- 优先读取 `round_gap_summary`（缺口摘要）。
- 结合 Judge 输出的 `next_checks / action_items` 选择相关专家。
- 若没有明确缺口，直接进入 Judge 收口。

这条规则落在 `StandardSecondRoundTargetedRule`，目的就是减少无效重复分析，提高收敛速度。
...
- 分析策略为 `quick` 或显式关闭验证门槛时，`VerificationAgent` 可能被策略跳过。
- `background` 仅表示投递方式，本身不会改变分析策略。
...
## 6.2 上下文封套（Context Envelope）

新版运行时不再把“全量会话对象”直接喂给每个专家，而是显式拆分成上下文封套，避免污染与超长。

封套层级：

- `shared_context`：全体共享的关键背景（故障摘要、关键信号、运行时策略）。
- `focused_context`：当前 Agent 的定向证据（责任田映射、重点代码、相关指标）。
- `tool_context`：工具与 Skill 注入结果（含审计摘要）。
- `peer_context`：其他专家的关键结论摘要。
- `mailbox_context`：主 Agent 或系统下发的私信指令。
- `work_log_context`：当前 Agent 的工作日志与历史假设。

这样做的收益：

- 控制输入体积，减少无效 token。
- 强化“每个专家有自己的上下文”和私有记忆隔离。
- 便于回放与审计（每次上下文都可追溯）。

关键入口：

- `backend/app/runtime/langgraph/context_builders.py`
...
运行时把“分析策略 / 预算 / 部署模板”拆成三层，避免在 orchestrator 内硬编码：

- **策略层**：解释 `execution_mode + analysis_depth_mode`
  - `backend/app/runtime/langgraph/runtime_policy.py`
- **预算层**：token / timeout / 轮次预算
  - `backend/app/runtime/langgraph/budgeting.py`
- **部署层**：是否启用 Metrics/Skill/协作子图等
  - `backend/app/runtime/deployment_center.py`
- **策略选择层**：把策略映射为 `runtime_strategy`
  - `backend/app/runtime/strategy_center.py`

这样做能让 `quick` 模式在弱模型/低并发场景下稳定收敛，同时让 `standard` 保持深度与质量。

### 6.3.1 策略与预算流转图

```mermaid
flowchart LR
    A["execution_mode + analysis_depth_mode"] --> B["runtime_policy.py"]
    B --> C["runtime_strategy (balanced/low_cost)"]
    C --> D["budgeting.py 预算配置"]
    C --> E["deployment_center.py 选择 deployment_profile"]
    D --> F["phase_executor / agent_runner"]
    E --> F
```
...
## 9.5 Runtime 内部状态流（新增）

```mermaid
flowchart TD
    A["state 初始化"] --> B["round_start"]
    B --> C["supervisor_decide"]
    C --> D{"next_step"}
    D -->|analysis_parallel| E["analysis_parallel_node"]
    D -->|speak:Agent| F["agent_node"]
    D -->|空| G["round_evaluate"]
    E --> C
    F --> C
    C --> G
    G --> H{"continue_next_round"}
    H -->|true| B
    H -->|false| I["finalize"]
    I --> J["output_state / final_payload"]
```

这个流程强调：state 的推进并不只是“调用 LLM”，而是“状态推进 + 路由 + 事件派发”的组合。
...
```mermaid
flowchart LR
    A["incident 输入"] --> B["责任田映射"]
    B --> C["interface_mapping"]
    B --> D["investigation_leads"]
    C --> E["focused_context"]
    D --> E
    E --> F["专家 Agent 输出证据"]
    F --> G["evidence_chain + claim_graph"]
    G --> H["Judge 收口"]
```
