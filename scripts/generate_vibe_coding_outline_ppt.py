...
OUTLINE_PATH = ROOT / "docs" / "plans" / "2026-03-06-vibe-coding-ppt-outline.md"
OUT_DIR = ROOT / "output" / "vibe-coding-outline-ppt"
ASSET_DIR = OUT_DIR / "assets"
OUT_PPTX = OUT_DIR / "2026-03-06-vibe-coding-practice-outline.pptx"
...
class Theme:
    ...
    ink_soft = RGBColor(72, 86, 103)
    gold = RGBColor(201, 147, 54)
...
def add_chart(
    ...
    *,
    ...
    palette = [Theme.blue, Theme.cyan, Theme.green, Theme.orange, Theme.red]
...
def add_footer_source(slide, source: str) -> None:
    """向当前页补充页脚来源相关元素，并统一样式与布局。"""
    
    box = slide.shapes.add_textbox(Inches(0.55), Inches(7.12), Inches(11.8), Inches(0.2))
    p = box.text_frame.paragraphs[0]
    p.text = f"Source: {source}"
    p.font.size = Pt(8.5)
    p.font.color.rgb = Theme.subtext


def add_pill(slide, x: float, y: float, w: float, text: str, fill: RGBColor, color: RGBColor = Theme.text) -> None:
    ...
    t = slide.shapes.add_textbox(Inches(x + 0.08), Inches(y + 0.07), Inches(w - 0.16), Inches(0.22))
...
def slide_1_cover(prs: Presentation) -> None:
    ...
    add_text(slide, 0.65, 0.6, 10.8, 0.85, "Vibe Coding 在生产级 SRE 智能体中的工程化落地", size=34, bold=True, color=Theme.white)
    add_text(
        slide,
        0.65,
        1.45,
    ...
    add_pill(slide, 0.7, 2.15, 1.9, "Harness First", Theme.light_blue, Theme.blue)
    add_pill(slide, 2.75, 2.15, 1.9, "Context Driven", Theme.light_cyan, Theme.cyan)
    add_pill(slide, 4.8, 2.15, 1.9, "Agent Governed", Theme.light_green, Theme.green)
    ...
    add_card(slide, 7.3, 5.85, 4.7, 0.5, Theme.white)
    add_text(slide, 7.45, 6.0, 4.4, 0.18, "Audience: Engineering leaders / SRE / Platform / AI teams", size=9, color=Theme.ink_soft)
    ...
    add_footer_source(slide, "Outline + OpenAI / Anthropic / GitHub / LangGraph / MCP references")
...
def slide_2_why(prs: Presentation) -> None:
    ...
    add_footer_source(slide, "OpenAI Harness Engineering; Thoughtworks vibe coding study")
...
def slide_3_harness(prs: Presentation) -> None:
    ...
    add_footer_source(slide, "OpenAI Harness Engineering")
...
def slide_4_context(prs: Presentation) -> None:
    ...
    add_footer_source(slide, "Anthropic prompt engineering; GitHub Copilot docs; Cursor rules")
...
def slide_5_workflow(prs: Presentation) -> None:
    ...
    add_footer_source(slide, "Anthropic Claude workflows; GitHub Copilot best practices")
...
def slide_6_gate(prs: Presentation) -> None:
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    add_bg(slide, prs)
    add_topbar(slide, "业界共识 4：测试和评测是 Vibe Coding 的刹车系统", "Quality gate as brakes", 6)
    ...
    add_footer_source(slide, "OpenAI eval-driven development; benchmark gate practice")
...
def slide_7_mcp(prs: Presentation) -> None:
    ...
    add_footer_source(slide, "MCP architecture spec; MCP security best practices")
...
def slide_8_multi_agent(prs: Presentation) -> None:
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    add_bg(slide, prs)
    add_topbar(slide, "业界共识 6：多 Agent 不是多线程，而是可控协作机制", "Protocol over parallel noise", 8)
    ...
    add_footer_source(slide, "LangGraph workflows-agents; supervisor pattern")
...
def slide_9_antipattern(prs: Presentation) -> None:
    ...
    add_footer_source(slide, "Thoughtworks production-grade vibe coding experiments")
...
def slide_10_project_harness(prs: Presentation) -> None:
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    add_bg(slide, prs)
    add_topbar(slide, "本项目案例 1：我们如何实现 Harness 化", "Runtime orchestration", 10)
    ...
    add_footer_source(slide, "Local project runtime: langgraph_runtime.py / builder.py / supervisor.py")
...
def slide_11_skill_tool(prs: Presentation) -> None:
    ...
    add_footer_source(slide, "Local project services: agent_tool_context_service.py / agent_skill_service.py")
...
def slide_12_resilience(prs: Presentation) -> None:
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    add_bg(slide, prs)
    add_topbar(slide, "本项目案例 3：韧性设计（超时、降级、恢复）", "Timeout, degrade, recover", 12)
    ...
    add_footer_source(slide, "Timeout, degrade and recovery design from local runtime implementation")
...
def slide_13_2week(prs: Presentation) -> None:
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    add_bg(slide, prs)
    add_topbar(slide, "团队落地模板（2 周可执行）", "From idea to operational practice", 13)
    ...
    add_footer_source(slide, "Execution template synthesized from outline and project plans")
...
def slide_14_roadmap(prs: Presentation) -> None:
    ...
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    add_bg(slide, prs)
    add_topbar(slide, "90 天路线图（从可用到规模化）", "P0 -> P3 capability maturity", 14)
    ...
    for p, name, desc, c in rows:
        ...
        y += 1.35
    add_footer_source(slide, "P0-P3 roadmap from outline and current RCA system roadmap")
...
def slide_15_close(prs: Presentation) -> None:
    ...
    add_footer_source(slide, "Conclusion synthesized from the 2026-03-06 vibe coding outline")
...
def write_slides_md() -> None:
    """写出幻灯片md相关产物。"""
    
    OUT_SLIDES.write_text(
        "# Vibe Coding 实践分享 Slides\n\n"
        "来源提纲：`docs/plans/2026-03-06-vibe-coding-ppt-outline.md`\n\n"
        "## Slide 1\n"
        "Vibe Coding 在生产级 SRE 智能体中的工程化落地\n\n"
        "## Slide 2\n"
        "为什么 Vibe Coding 值得做：价值在缩短认知到交付闭环。\n\n"
        "## Slide 3\n"
        "Harness Engineering 是核心：模型负责推理，系统负责控制。\n\n"
        "## Slide 4\n"
        "上下文工程优先于长 Prompt：结构化、分步、最小相关上下文。\n\n"
        "## Slide 5\n"
        "标准工作流替代即兴开发：Explore -> Plan -> Build -> Verify -> Release。\n\n"
        "## Slide 6\n"
        "测试和评测是刹车系统：测试、benchmark、CI gate 缺一不可。\n\n"
        "## Slide 7\n"
        "MCP/工具化接入必须标准化：统一接口、权限、审计。\n\n"
        "## Slide 8\n"
        "多 Agent 的价值在协议与收敛，不在数量。\n\n"
        "## Slide 9\n"
        "业界反模式：Accept-All、上下文堆砌、无审计、无终态。\n\n"
        "## Slide 10\n"
        "项目案例 1：Runtime、Graph、Supervisor 分层实现 Harness。\n\n"
        "## Slide 11\n"
        "项目案例 2：Command -> Gate -> Tool -> Skill -> LLM -> Audit。\n\n"
        "## Slide 12\n"
        "项目案例 3：超时、重试、降级、恢复，保证不会长期 pending。\n\n"
        "## Slide 13\n"
        "团队落地模板：2 周内先做可用性、可观测和门禁。\n\n"
        "## Slide 14\n"
        "90 天路线图：P0 可用性 -> P1 准确性 -> P2 可控自治 -> P3 持续学习。\n\n"
        "## Slide 15\n"
        "结尾：Vibe Coding 的终局是可控地持续交付正确结果。\n",
        encoding="utf-8",
    )
...
def write_notes_md() -> None:
    ...
    OUT_NOTES.write_text(
        "# Speaker Notes\n\n"
        "受众：研发负责人、SRE、平台工程、AI 工程团队\n"
        "目标：解释如何把 Vibe Coding 做成可落地的工程方法。\n\n"
        "## Slide 1\n"
        "开场先定边界：这不是炫技式 AI 编码，而是强调可治理、可复用、可上线的工程实践。\n\n"
        "## Slide 2\n"
        "说明 Vibe Coding 的真实价值是缩短认知到交付闭环，但如果没有约束，速度会放大错误。\n\n"
        "## Slide 3\n"
        "强调 OpenAI 提出的 Harness 思想：不要把希望压在单次 Prompt 上，而要建立控制系统。\n\n"
        "## Slide 4\n"
        "讲上下文工程：给模型正确、适量、结构化的上下文，比把所有信息堆进去更有效。\n\n"
        "## Slide 5\n"
        "解释固定工作流如何降低返工：先 Explore，再 Plan，再 Build，最后 Verify 和 Release。\n\n"
        "## Slide 6\n"
        "测试与评测是刹车系统，重点提 benchmark、CI Gate 和可量化指标。\n\n"
        "## Slide 7\n"
        "讲 MCP 不是单纯工具接入，而是标准化协议、权限边界、可审计调用。\n\n"
        "## Slide 8\n"
        "讲多 Agent 的关键是命令协议、证据交换和停止条件，避免伪多 Agent。\n\n"
        "## Slide 9\n"
        "总结最容易踩的四类坑，并说明这些坑都不是模型能力问题，而是工程纪律问题。\n\n"
        "## Slide 10\n"
        "用本项目说明 Harness 如何落到运行时：主 Agent 先发命令、专家后执行、结果可回放。\n\n"
        "## Slide 11\n"
        "展示 Skill 和 Tool 的调用链，强调可控、可审、可解释。\n\n"
        "## Slide 12\n"
        "讲韧性：超时不等于失败，关键是如何重试、降级、恢复，避免系统卡死。\n\n"
        "## Slide 13\n"
        "给听众一个两周内可执行的落地模板，降低启动门槛。\n\n"
        "## Slide 14\n"
        "说明 Vibe Coding 最终会走向平台化治理，而不是停留在个人效率工具。\n\n"
        "## Slide 15\n"
        "收束到行动清单：规范、审计、benchmark、可回滚、真实演练。\n",
        encoding="utf-8",
    )


def write_refs_md() -> None:
    """写出参考资料md相关产物。"""
