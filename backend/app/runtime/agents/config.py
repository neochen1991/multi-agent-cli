...
本模块定义了所有 Agent 的配置，包括：
1. AgentConfig 数据类：定义 Agent 的配置结构
2. AGENT_CONFIGS 字典：所有 Agent 的配置定义
3. 辅助函数：获取 Agent 配置

Agent 类型说明：
- ProblemAnalysisAgent: 问题分析主Agent，负责调度协调
- LogAgent: 日志分析专家
- DomainAgent: 领域映射专家
- CodeAgent: 代码分析专家
- DatabaseAgent: 数据库取证专家
- MetricsAgent: 监控指标专家
- ChangeAgent: 变更关联专家
- RunbookAgent: 处置手册专家
- RuleSuggestionAgent: 告警规则建议专家
- CriticAgent: 架构质疑专家
- RebuttalAgent: 技术反驳专家
- JudgeAgent: 技术委员会主席
- VerificationAgent: 验证计划专家
...
class AgentConfig:
    ...
    - name: Agent 唯一标识符
    - role: Agent 角色描述
    - phase: Agent 所属阶段（coordination/analysis/critique/rebuttal/judgment/verification）
    - system_prompt: 系统提示，定义 Agent 的行为规范
    ...
    role: str  # Agent 角色描述
    phase: str  # Agent 所属阶段
    system_prompt: Optional[str] = None  # 系统提示
    tools: List[str] = field(default_factory=list)  # 可用工具列表
    max_tokens: int = 320  # 最大输出 token 数
    timeout: int = 35  # 执行超时（秒）
    temperature: float = 0.15  # LLM 温度
    ...
    def to_spec(self) -> "AgentSpec":
        ...
        AgentSpec 是运行时使用的不可变 Agent 规格。

        Returns:
            AgentSpec: Agent 规格实例
        """
...
def _prompt(role_specific: str) -> str:
    ...
    将通用护栏和角色特定提示组合成完整的系统提示。

    Args:
        role_specific: 角色特定的提示内容
...
# 所有 Agent 的配置字典
# 每个配置包含：name, role, phase, system_prompt, tools, max_tokens, timeout
...
def get_agent_config(name: str) -> Optional[AgentConfig]:
    ...
    Returns:
        Optional[AgentConfig]: Agent 配置，如果不存在则返回 None
    """
...
def get_all_agent_configs() -> Dict[str, AgentConfig]:
    ...
    Returns:
        Dict[str, AgentConfig]: Agent 名称到配置的映射
    """
...
def get_agents_by_phase(phase: str) -> List[AgentConfig]:
    ...
    Args:
        phase: 阶段名称（coordination/analysis/critique/rebuttal/judgment/verification）

    Returns:
        List[AgentConfig]: 该阶段的所有 Agent 配置列表
    """
    return [
...
def get_enabled_agent_configs() -> List[AgentConfig]:
    ...
    Returns:
        List[AgentConfig]: 所有启用的 Agent 配置列表
    """
    return [
        AgentConfig.from_dict(config)
        for config in AGENT_CONFIGS.values()
