...
class ReportGenerationService:
    ...
    async def _generate_report_with_ai(
        ...
        """调用 LLM 生成结构化报告内容，失败时由上层兜底为模板报告。"""
        try:
    ...
    def _build_report_prompt(
        ...
        """构建报告生成提示，并对 incident/debate/assets 做压缩摘要。"""
...
executive_summary、incident_overview、root_cause_analysis、impact_assessment、recommendations、lessons_learned、appendix。"""
    ...
    def _report_ai_skip_reason(self, debate_result: Dict[str, Any]) -> Optional[str]:
        """执行报告aiskipreason相关逻辑，并为当前模块提供可复用的处理能力。"""
        if not isinstance(debate_result, dict):
            return "辩论结果不可用"
        final_judgment = debate_result.get("final_judgment")
    ...
    def _report_json_schema(self) -> Dict[str, Any]:
        """执行报告jsonSchema相关逻辑，并为当前模块提供可复用的处理能力。"""
        return {
            "type": "object",
            "properties": {
    ...
    def _safe_dict(value: Any) -> Dict[str, Any]:
        """执行safedict相关逻辑，并为当前模块提供可复用的处理能力。"""
        return value if isinstance(value, dict) else {}

    @staticmethod
    def _safe_list(value: Any) -> List[Any]:
        """执行safe列出相关逻辑，并为当前模块提供可复用的处理能力。"""
...
<body>
    ...
    def _format_responsibility_mapping_markdown(self, mapping: Dict[str, Any], assets: Dict[str, Any]) -> str:
        """执行formatresponsibilitymappingmarkdown相关逻辑，并为当前模块提供可复用的处理能力。"""
